...
export default function BottomNav() {
  ...
  const resolvedActive = pathname === "/wishlist"
    ? "wishlist"
    : pathname === "/profile"
      ? "profile"
      : pathname === "/categories"
  ...
  return (
    <>
      <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-stone-200/70 shadow-[0_-3px_10px_rgba(0,0,0,0.04)] font-sans">
        <div className="max-w-md mx-auto flex items-center justify-around px-2 py-0.5 h-12 relative">
          {navItems.map(({ id, label, Icon, badge, isSpecial, href }) => {
            const isActive = resolvedActive === id || (id === "search" && isSearchOpen);

            // Compact Floating Bump Search button
            if (isSpecial) {
              return (
                <button
                  key={id}
                  ...
                  className="flex flex-col items-center justify-center -mt-4 focus:outline-none relative z-10"
                >
                  <div
                    className={`
                  ...
                  <span
                    className={`text-[9px] mt-0.5 font-bold transition-colors ${
                      isActive ? "text-stone-950 font-black" : "text-stone-400 font-medium"
                    }`}
                  >
                    {label}
                  </span>
            ...
            // Standard nav items with dynamic yellow badge
            return (
              <Link
                key={id}
                href={href || "/"}
                onClick={() => setActive(id)}
                className="flex flex-col items-center justify-center px-2 py-0.5 transition-all relative"
              >
                {/* Sleek top yellow indicator line */}
                {isActive && (
                  <span className="absolute -top-1 w-6 h-0.5 bg-[#facc15] rounded-full" />
                )}
                  ...
                  <Icon
                    className={`w-4.5 h-4.5 transition-all ${
                      isActive
                        ...
                        : "text-stone-400 stroke-[1.7]"
                    }`}
                  />
                  {badge && badge > 0 ? (
                    <span className="absolute -top-1.5 -right-2.5 min-w-[15px] h-[15px] px-1 bg-[#facc15] text-stone-950 text-[8.5px] font-black rounded-full flex items-center justify-center border border-white shadow-2xs">
                      {badge}
                    </span>
                  ) : null}
                </div>

...
import { Sparkles, CheckCircle2, Star, PackageCheck } from 'lucide-react';
...
export default function ThemeStatistics({ themes }: Props) {
  const total = themes.length;
  const active = themes.filter((t) => t.status === 'Active').length;
  const featured = themes.filter((t) => t.featured).length;
  const totalProducts = themes.reduce((acc, t) => acc + t.productsCount, 0);

  const stats = [
    {
      label: 'Total Themes',
      value: total,
      subtext: 'Artwork design concepts',
      icon: Sparkles,
      bgColor: 'bg-[#facc15]',
    ...
    },
    {
      label: 'Active Themes',
      value: active,
      subtext: `${total ? Math.round((active / total) * 100) : 0}% live in catalog`,
      icon: CheckCircle2,
    ...
    },
    {
      ...
      value: featured,
      subtext: 'Showcased on landing pages',
      icon: Star,
    ...
    },
    {
      ...
      subtext: 'Items mapped to themes',
      icon: PackageCheck,

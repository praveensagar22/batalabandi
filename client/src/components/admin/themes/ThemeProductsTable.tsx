...
import { ThemeProductItem } from '@/lib/themes/types';
...
export default function ThemeProductsTable({ products, onRemoveProduct }: Props) {
  return (
            ...
            {products.length > 0 ? (
              products.map((p) => (
                <tr key={p.id} className="hover:bg-stone-50/70 transition">
                  ...
                  <td className="p-3 font-semibold text-stone-800">{p.productType}</td>
                  <td className="p-3 font-semibold text-amber-900">{p.collection}</td>
              ...
              <tr>
                <td colSpan={8} className="py-8 text-center text-stone-400 text-xs">
                  No products currently linked to this theme.
                </td>
              </tr>
            )}
          </tbody>

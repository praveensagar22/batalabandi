...
import { CollectionFilterState } from '@/lib/collections/types';
...
export default function CollectionSearchAndFilters({
  filters,
  viewMode,
  onViewModeChange,
  onChange,
  ...
  return (
    <div className="bg-white border border-stone-200/80 rounded-2xl p-4 shadow-sm space-y-3">
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select
            ...
            onChange={(e) =>
              onChange({ ...filters, status: e.target.value as CollectionFilterState['status'] })
            }
          ...
          <select
            ...
            onChange={(e) =>
              onChange({ ...filters, featured: e.target.value as CollectionFilterState['featured'] })
            }
            className="bg-stone-50 border border-stone-200 text-stone-800 text-xs font-medium rounded-xl px-3 py-2 outline-none focus:border-yellow-400 transition"
          >
          ...
          <select
            ...
            onChange={(e) =>
              onChange({ ...filters, sortBy: e.target.value as CollectionFilterState['sortBy'] })
            }
            className="bg-stone-50 border border-stone-200 text-stone-800 text-xs font-medium rounded-xl px-3 py-2 outline-none focus:border-yellow-400 transition"
          >
            <option value="name">Sort by Name</option>
            <option value="productsCount">Sort by Products Count</option>
            <option value="createdDate">Sort by Recently Created</option>
          </select>
      ...
      <div className="flex items-center justify-between text-xs text-stone-500 pt-1">
        <span>
          Showing <strong className="text-stone-900">{totalResults}</strong> marketing collections
        </span>
